...
Lens::Lens(Vector2f filmSize, Vector3f position, Vector3f lookAt, Vector3f upVector, float fov, float exposure) : Camera(filmSize), position(position), lookAt(lookAt), upVector(upVector), fov(fov), exposure(exposure) {};

Lens Lens::fromJson(const nlohmann::json &json) {
  return Lens(Vector2f(json["width"], json["height"]), json["position"], json["lookAt"], json["upVector"], json["fov"], json["exposure"]);
}
